...
terraform {
 backend "s3" {
   bucket = "rtccpro1"
   key    = "SanJose/Instances/GWDir2/terraform.tfstate"
   region = "us-west-2"
 }
}
...
resource "aws_ebs_volume" "training-GWDir2-Volume2" {
  availability_zone = "us-west-1c"
  size              = 100
  encrypted         = true
  tags {
        Name = "training-GWDir2-Volume2"
    }
}

resource "aws_instance" "training-GWDir2" {
...
resource "aws_volume_attachment" "ebs_att-FDir" {
  ...
  volume_id   = "${aws_ebs_volume.training-GWDir2-Volume2.id}"
  instance_id = "${aws_instance.training-GWDir2.id}"
}
