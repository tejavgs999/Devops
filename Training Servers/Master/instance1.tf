...
terraform {
 backend "s3" {
   bucket = "rtccpro1"
   key    = "SanJose/Instances/Master/terraform.tfstate"
   region = "us-west-2"
 }
}
*/

resource "aws_ebs_volume" "training-master-Volume2" {
  availability_zone = "us-west-1a"
  size              = 100
  encrypted         = true
  tags {
        Name = "training-master-Volume2"
    }
}


resource "aws_instance" "training-master" {
...
resource "aws_volume_attachment" "ebs_att-master" {
  ...
  volume_id   = "${aws_ebs_volume.training-master-Volume2.id}"
  instance_id = "${aws_instance.training-master.id}"
}
