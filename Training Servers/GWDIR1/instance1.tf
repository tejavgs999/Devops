...
terraform {
 backend "s3" {
   bucket = "rtccpro1"
   key    = "SanJose/Instances/GWDir1/terraform.tfstate"
   region = "us-west-2"
 }
}
*/

resource "aws_ebs_volume" "training-GWDIR1-Volume2" {
  availability_zone = "us-west-1a"
  size              = 100
  encrypted         = true
  tags {
        Name = "training-GWDIR1-Volume2"
    }
}

resource "aws_instance" "Training-GWDIR1" {
...
resource "aws_volume_attachment" "ebs_att-GWDIR1" {
  device_name = "/dev/sda2"
  volume_id   = "${aws_ebs_volume.training-GWDIR1-Volume2.id}"
  instance_id = "${aws_instance.Training-GWDIR1.id}"
}
